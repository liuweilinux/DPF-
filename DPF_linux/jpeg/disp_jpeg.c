...
{
        char *path = "../jpeg/constellation/";
        char *p = (char *)fb_inf.fbmem;
        int i,j,a = 1;
        char filename[50];
        strcpy(filename, path);
        for(i = 30; i < 1024; i+=1)
        {
                sprintf(filename + strlen(path),"a%d.jpg", a);
                fb_inf.fbmem = p + (1024*567 + j)*fb_inf.bpp/8;
                fb_inf.w = 200;
                fb_inf.h = 200;
                disp_background(filename, fb_inf);
                a++;
        }
        print_page(fb_inf);
}
